...
void setup() {
  ...
  setGridPosition(0, 1); // Grid position for USV3
  ...
  if (rf96.available()) {
    ...
    if (rf96.recv(buf, &len)) {
      if (strcmp((char *)buf, "SOS") == 0) {
        ...
        if (!isCentralUSV) {
          isCentralUSV = false; // USV3 is not the central USV
        }
        convergeOnSignal();
      }
    }
